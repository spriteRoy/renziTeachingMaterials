...
**`本节注意`**:修改服务的配置文件,想要生效的话,必须要**`重新启动服务`**，值‘8888’后面**`不能留有空格`**
...
@keyup.enter.**`native`** 表示监听组件的原生事件，比如 keyup就是于input的原生事件（el-input没有input事件，想要监听input事件就需要加native），这里写native表示keyup是一个原生事件
...
开发环境的跨域，也就是在**`vue-cli脚手架环境`**下开发启动服务时，我们访问接口所遇到的跨域问题，vue-cli为我们在本地**`开启了一个服务`**（本项目的本地服务的后端）,可以通过这个服务帮我们**`代理请求`**,解决跨域问题
...
**`vue.config.js`**的改动如果要生效,需要进行**`重启服务`**
...
export function login(data) {
  ...
  return request({
    ...
    method: 'post',
      // body参数在axios请求是放在data属性中的（params是路径参数）
    data
  })
}
```
...
## 封装Vuex的登录Action并处理token （在登录页面中加入vuex）
...
> OK，除此之外,axios返回的数据中默认增加了一层**`data的包裹`**,一般会在axios响应拦截器中处理。

> 并且,人资项目的接口,如果执行失败,只是设置了**`success`**变量为**`false`**，并没有reject，我们需要一并处理下
